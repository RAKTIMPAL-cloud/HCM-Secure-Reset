...
st.title("👤 Oracle HCM: Precise User & Role Management")
...
if st.button("🚀 Process Bulk Operations"):
    if not (username and password and uploaded_file):
        st.warning("Please provide all credentials and the file.")
    else:
        df = pd.read_excel(uploaded_file)
        df.columns = [c.strip().upper() for c in df.columns]
        common_pwd = generate_secure_password()
        
        # PHASE 1: CREATE USERS (Unique only)
        ...
        with st.spinner("⏳ Step 1: Processing User Creation..."):
            ...
            if u_res.status_code in [200, 201]:
                # PARSING INDIVIDUAL USER RESULTS
        ...
        if csv_data:
            ...
            for _, row in df.iterrows():
                ...
                # We need a unique bulkId per row to map results back correctly
                unique_bulk_id = f"assign_{u_name}_{secrets.token_hex(3)}"
                
                if u_guid and r_guid:
                    role_ops.append({
                        "method": "PATCH", "path": f"/Roles/{r_guid}", "bulkId": unique_bulk_id,
                        "data": {"members": [{"value": u_guid, "operation": "ADD"}]}
                    })
                    tracking_list.append({
                        "USER NAME": u_name, 
                        "ROLE": r_name, 
                        "USER_STATUS": user_status_results.get(u_name, "N/A"), 
                        "BULK_ID": unique_bulk_id
                    })
                else:
                    tracking_list.append({
                        "USER NAME": u_name, 
            ...
            if role_ops:
                with st.spinner("⏳ Step 3: Assigning Roles..."):
                    ...
                    # PARSING INDIVIDUAL ROLE RESULTS
            ...
            st.info(f"🔑 Password for new users: `{common_pwd}`")
            
            final_df = pd.DataFrame(tracking_list).drop(columns=['BULK_ID'])
            st.table(final_df)
        else:
            st.error("🚨 Error: Could not retrieve GUIDs from Oracle. Please verify the BIP report.")
